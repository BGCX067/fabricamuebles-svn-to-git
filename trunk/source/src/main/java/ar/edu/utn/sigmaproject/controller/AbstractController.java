...
public abstract class AbstractController<E extends IdentificableEntity> {
    ...
    public boolean confirmSave(MessageContext messageContext, Locale locale) {
        if (getModel().isNewMode() && getModel().getEditedEntity().getId() != null) {
            // We nullify the id, in case of a previous failed attempt to save this new entity
            getModel().getEditedEntity().setId(null);
        }
        bindModelToEntity();
